...
for e in "${endpoints_array[@]}"; do
    ...
    if (unset -v ETCDCTL_ENDPOINTS; etcdctl endpoint health "${extra_flags[@]}"); then
        info "Snapshotting the keyspace"
        current_time="$(date -u "+%Y-%m-%d_%H-%M")"
        (unset -v ETCDCTL_ENDPOINTS; etcdctl snapshot save "/snapshots/db-${current_time}" "${extra_flags[@]}")
